import discord, requests, pickle, sys, os
...
guild_ids = [434994995410239488, 221049808784326656] #VORK, ENWIKI

class Slash(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    #@cog_ext.cog_slash(name="invote", description='TEST COMMAND - Get the link to invite this bot to your server')
    #async def _est(self, ctx: SlashContext):
    #    #embed = discord.Embed(title="embed test")
    #    await ctx.respond(eat=True)
    #    await ctx.send(content="https://discord.com/oauth2/authorize?client_id=626409329905631263&permissions=2147483648&scope=bot%20applications.commands", hidden=True)#, embeds=[$
    ...
    @cog_ext.cog_slash(name="whois", 
                ...
                options=[
                    create_option(
                        name="user",
                        description="The user you desire to search.",
                        option_type=6 or 4,
                        required=False
                    )
                ])
